...
class FlowPanel {
  ...
  _dataSectionHTML() {
    ...
    const practiceOptions = practice.map((d) => `<option value="${escapeHtml(d.groupId)}|${escapeHtml(d.id)}">${escapeHtml(d.name)}</option>`).join('');
    const t = this.table;
  ...
  _locationSectionHTML() {
    ...
    // 이름 열이 비어 있는 피처는 layerLocations 가 조용히 빼므로, 몇 개가 빠졌는지 알려 준다
      ...
      <div class="flow-section">
        ...
        ${this.locationSource === 'layer' ? `
          ...
          ${base ? `
            ...
            ${excluded > 0 ? `<div class="flow-muted">이름 없는 피처 ${excluded}개 제외</div>` : ''}` : ''}
        ` : `
          <div class="form-group">
            <label>위치 표</label>
  ...
  _matchSectionHTML() {
    ...
    const unmatchedRows = shown.map((u) => `
      <div class="flow-unmatched-row">
        ...
        ${editing ? '' : `<select data-manual="${escapeHtml(u.name)}"><option value="">(제외)</option>${options}</select>`}
      </div>`).join('');
    return `
      <div class="flow-section">
        ...
        ${editing ? '' : `<div class="form-group"><label>레이어 이름</label><input type="text" id="flow-title" value="${escapeHtml(this.title)}"></div>`}
      </div>`;
  }
