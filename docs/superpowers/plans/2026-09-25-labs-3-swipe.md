...
- **Task 3 리뷰 반영 — 컨트롤 박스 위치.** 계획의 `.swipe-controls { top: 12px; right: 12px }` 는 오른쪽 위 지도 컨트롤 열(`.ol-zoom`, `.egis-compass/geolocate/basemap` — `right: var(--spacing-sm)`, 폭 32px, top 76/116/156)과 `.view3d-panel` 자리를 그대로 덮는다. 3D 는 지도를 덮어 문제가 없지만 스와이프는 지도를 그대로 쓰므로 비교하는 동안 확대·배경지도 버튼을 못 쓰게 된다. 그래서 `right: calc(var(--spacing-sm) + 32px + 12px)` 로 그 열의 왼쪽에 둔다(`top: 12px`, z-index 30 은 유지). 글래스 데스크톱 블록의 `top: calc(var(--glass-top-offset) + 12px)` 는 그대로(right 는 글래스와 무관). 같은 Task 에서 글래스 규칙도 더했다: `.swipe-panel` 을 부유 블러 목록 두 곳(기본·`pointer: coarse` 블러 없음)에, 데스크톱 블록에 `.swipe-controls` top 오프셋과 막대 여백(세로: `margin-top`·`height` 로 위·아래 막대를 피함, 가로: `margin-left`·`width` 로 왼쪽 패널을 피함 — top/left 는 JS 가 인라인 % 로 쓰므로 여백·크기로 민다).
- **Task 4/5 에 넘기는 것 — 스와이프와 3D 는 서로 배타.** 3D 는 지도를 덮고 컨트롤 자리도 겹치므로 동시에 열지 않는다. `main.js` 의 `case 'view3d'` 가 3D 를 켤 때 열린 스와이프를 닫고, 스와이프를 열 때 3D 가 이미 켜져 있으면 스와이프 박스를 열지 않거나(또는 닫고) 안내한다.
- **후속 과제 — 글래스에서 막대 비율 제한.** 글래스 데스크톱에서는 지도가 창 전체라 비율 0~1 이 메뉴·툴바·상태줄·왼쪽 패널 밑까지 닿는다. 막대가 보이는 지도 칸 안에만 머물도록 패널이 비율을 `--glass-*-offset` 기준으로 제한하는 건 이번 범위 밖, 뒤에 한다.
- 최종 검토 보정(2026-09-26): 흐름도(`type:'flow'`, `FlowRenderer` 커스텀 Layer)는 prerender/postrender 가 안 떠서 클립이 안 되므로 히트맵과 함께 대상 목록에서 뺀다(`UNCLIPPABLE_TYPES`). 병합 뒤 `glass.css` 광채 목록에 `.swipe-panel`·버튼 목록에 `.swipe-close` 추가.
