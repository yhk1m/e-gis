...
- **Task 10 하네스가 찾은 결함 2건** — (1) `ClassFillPopover`: `setClassFill → restyle → refreshLegendItems` 가 범례 항목 `innerHTML` 을 통째로 갈아 끼우므로 `open()` 때 받은 anchor 가 곧 떨어져 나간 노드가 됐다. rect 가 전부 0 이라 패턴·질감 탭으로 바꾸는 순간 팝오버가 `left:-256px, top:-8px` 로 튀어 왼쪽 패널 뒤에 가려졌고(`class-fill-02-hatch.png`), `anchor.contains()` 도 새 칸을 몰라 바깥 클릭 예외가 깨졌다. `resolveAnchor()` 가 `#choropleth-legend-{layerId} .choropleth-legend-color[data-class="{i}"]` 로 지금 문서의 칸을 다시 찾아 `this.anchor` 를 갱신하고(못 찾으면 갖고 있던 것), `position()` 과 바깥 클릭 처리기가 이를 거친다. 그 결과 **다른 구간의 칸은 바깥**으로 친다(기존 테스트가 classIndex 0 을 열며 1번 칸을 anchor 로 넘기던 불일치를 바로잡음). (2) `ChoroplethTool.renderLegendItems`: 배경 '없음' 패턴도 칸에 `background-color: colors[i]` 를 깔아 지도·내보내기 범례(배경지도가 비침)와 달랐다. 순수 헬퍼 `swatchStyle(fill, color, tileUrl)` 로 빼서 `planFill` 의 배경 규칙을 따른다 — 없음이면 `transparent`(타일이 없을 때도 지도 fallback 처럼 투명), 그 밖에는 구간 색. 테스트: 팝오버 14 → 15, 범례 8 → 13.
...
- 최종 검토 보정(2026-09-26): `classFillColor` 가 `fillFor` 의 `pixelScale` 로 `window.devicePixelRatio` 를 넘긴다 — OL 9 는 벡터 캔버스를 기기 픽셀로 그리면서 raw `CanvasPattern` 을 pixelRatio 로 키우지 않아(`ol/render/canvas/Builder` `fillPatternScale = 1`), Windows 125% 배율에서 지도 무늬가 범례·내보내기보다 촘촘해지던 것을 맞췄다. 고정 규약의 시그니처는 그대로(4번째 인자만 1 → DPR).
- 실험이 꺼져 있어도 달라지는 점(Task 3 통합의 결과, 의도됨): 단계구분도를 새로 만들면 테두리 두께가 처음부터 1 로 심어진다(예전엔 1 이었다가 투명도를 만지면 2 로 바뀌던 불일치 해소), 값 없는(NaN) 피처의 채움 불투명도가 0.5 → 0.7(레이어 불투명도를 따름).
