...
- 후속(2026-09-26, 사용자 결정): 왼쪽 패널을 지도 위에 띄웠다. (A) `glass.css` 에 데스크톱 전용 `@media (min-width: 1025px) and (pointer: fine)` 블록 — `#main-container` relative, `#map-container` 를 absolute `inset:0` 으로 깔고 `#left-panel`·`.panel-resizer` 를 그 위에(relative + `--z-panel`). 왼쪽에 붙는 부유 요소(`.sidebar-toggle`, `.choropleth-legend`·`.heatmap-legend`·`.isochrone-legend`·`.feature-info-card`, 자체 축척바 `.map-scale-bar` — OL `.ol-scale-line` 은 안 쓰는 걸로 확인)는 `--glass-panel-offset` 만큼 민다. 태블릿 bottom sheet 분기는 제외. (B) `glass.js` 에 `panelOffset`(순수)·`trackPanelOffset`(ResizeObserver 로 패널 너비·숨김을 따라가 `#map-container` 의 `--glass-panel-offset` 을 갱신하고 `resize` 를 쏴 OL 이 크기를 다시 잰다) 추가, `bindGlass` 가 켜질 때 추적·꺼질 때 해제(노드 테스트는 deps 주입, `document` 없으면 건너뜀). 스펙의 "JS 변경 없음" 은 더는 맞지 않는다 — JS 는 패널 오프셋 추적뿐. (C) 하네스에 3b(지도가 패널 아래까지·토글 위치·오프셋 변수·숨김 토글 왕복·축척바)·3c(단계구분도 범례가 패널 오른쪽에, `labs-03b-glass-legend.png`)·6(끄면 flow 복귀·변수 제거) 검사 추가. `getBoundingClientRect()` 의 DOMRect 는 `executeJavaScript` IPC 로 안 넘어오므로 `.toJSON()` 으로 넘긴다.
- 후속 v2(2026-09-26, 사용자 피드백 "그냥 음영만"): 원인은 둘. (1) 사용자의 Chrome 152 에서 `prefers-reduced-transparency: reduce` 가 참(Windows "투명 효과" 꺼짐)이라 `glass.css` 의 대체 규칙이 패널을 94% 불투명·블러 없음으로 만들었다. (2) 레시피가 약했고(0.62/14px) 메뉴바·툴바·상태줄 뒤에는 지도가 없었다. 바꾼 것: (A) `glass.css` — 토큰을 라이트 .42/다크 .45(막대는 .72)·블러 18/12px·`saturate(170%)`·광택 그라디언트 `--glass-sheen`·`--glass-shadow` 로 강화, 팝오버(드롭다운·컨텍스트 메뉴·검색 결과)는 `--glass-popover-bg` 로 진하게(안 그러면 패널 글자가 메뉴 글자 사이로 비친다), `prefers-reduced-transparency` 블록 삭제(직접 켠 실험이라 무시), 태블릿 무블러 분기는 `.78/.7` 로 올림. 데스크톱 블록은 `#app` 을 기준으로 `#map-container` 를 창 전체에 깔고 메뉴바·툴바·상태줄·패널 카드(8px 여백·12px 둥근 모서리)를 그 위에 — 부유 요소는 `--glass-panel/top/bottom-offset` 세 변수로 민다(줌·나침반·GPS·배경지도·3D 컨트롤·피처 카드는 위, 범례·축척·attribution 은 아래, 사이드바 토글은 보이는 띠 가운데). `.modal` 은 대화상자 본체(`.modal-overlay` 가 오버레이)라 블러 목록에 그대로 둔다. (B) `glass.js` — `panelOffset`/`trackPanelOffset` 을 `layoutOffsets`(순수, rect 기반)·`trackLayoutOffsets`(패널과 main 을 함께 관찰 — 툴바 접기 반영)로 일반화, `OFFSET_PROPS` 내보냄. 테스트 12개로 교체. (C) 하네스 — 지도가 `#app` 과 같은 rect, 세 오프셋 값, 줌·축척·범례가 막대 안쪽, 툴바 접기 왕복, 패널 숨김 왕복, 메뉴바 드롭다운이 패널 위(`elementFromPoint`, `labs-03c-glass-menu.png`), 끄면 `labs-05-off` 가 `labs-01` 과 픽셀 동일(상태줄 조회수 제외) 검사. 3c 에서 불러온 레이어가 자동 저장돼 재로드 때 "이전 작업 복원" 창이 캡처를 가렸으므로 로드 뒤 `#restore-no` 를 누른다. `vite preview` 는 4173 이 다른 세션에 잡혀 있으면 4174 로 뜨니 `EGIS_URL` 로 넘긴다. 알려진 한계: 글래스가 켜진 동안 `html2canvas(mapElement)` 내보내기에 패널·막대 아래 가려진 지도까지 찍힌다 — 실험 범위에서는 고치지 않고 사용설명서에 안내. 다크 모드에서 밝은 배경지도 위 `--text-secondary`(툴바 저작권 문구 등)는 여전히 흐리다.
- 글래스 v2 후속(2026-09-26, 사용자 요청): 메뉴바·툴바·상태줄도 왼쪽 패널처럼 8px 여백 + 12px 라운드 유리 카드로. 위·아래 오프셋은 rect 기준이라 CSS 만 바꿨다. 우상단 컨트롤 겹침 신고는 시제품(나침반·위치 오프셋 누락) 때 것으로, 배포판 측정에서는 겹치지 않았다.
- 글래스 후속 2(2026-09-26, 사용자 요청): 카드 간격을 `--glass-gap: 4px` 로 절반, 툴바·메뉴바·패널 탭·사이드바 토글·OL 컨트롤 버튼에 호버(밝아짐 + 위쪽 하이라이트)·누름(안쪽 그림자 + 1px 내려감) 반응 추가. `:active` 는 `:not(.active):hover` 보다 우선순위가 낮아 `:hover:active` 짝을 함께 둔다. 하네스의 끄기 전후 비교는 타일 글자 안티앨리어싱 차이 때문에 채널 차 ≤12·0.1% 미만 허용으로 완화.
