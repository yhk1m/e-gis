...
- **불투명 배경(Task 11 하네스에서 발견)**: 캔버스가 투명해 구 밖으로 OSM 타일·2D 단계구분도가 비쳤다. `--globe-bg` 토큰(라이트 `#eef2f7`, 다크 `#0b1220`)을 두고 `paint` 가 바다보다 먼저 `fillRect(0, 0, w, h)` 로 칠한다. `.globe-canvas { background: var(--globe-bg) }` 도 보험으로 둔다. PNG 저장(`toDataURL`)만 `transparent: true` 로 한 번 그려 구 밖을 투명하게 담고, 화면은 곧바로 배경까지 다시 그린다.
- **더블클릭 선택·드래그앤드롭 방지(Task 11)**: 더블클릭이 옆 글자를 선택해 두면 다음 드래그가 네이티브 `dragstart` → `pointercancel` 로 끊겼다. 캔버스 `draggable = false`, `user-select: none`, 포인터 `pointerdown` 의 `preventDefault()`(`touch-action: none` 이라 핀치는 영향 없음), `dragstart` 막기, `dblclick` 에서 `getSelection().removeAllRanges()`.
- **글래스 여백만큼 맞춤(Task 11)**: 글래스 데스크톱(`data-surface="glass"` + `(min-width: 1025px) and (pointer: fine)`)에서는 구가 반투명 막대·패널 밑으로 들어갔다. `projections.make/fitScale` 에 `inset {top,right,bottom,left}` 을 더해 맞춤 상자를 줄이고, 컨트롤러가 `#map-container` 의 `--glass-top-offset`·`--glass-bottom-offset`·`--glass-panel-offset` 을 읽어 넘긴다. glass.js 가 오프셋이 바뀔 때 쏘는 window `resize` 와 ResizeObserver 로 다시 잰다.
- **2D 축척바 숨김(Task 11)**: 지구본이 켜진 동안 `#map-container.globe-active` 를 두고 `.map-scale-bar` 를 숨긴다(지구본에서 "2 km" 같은 값은 틀린 정보). 단계구분도 범례는 지구본에서도 유용해 그대로 둔다.
