...
export class FlowRenderer extends Layer {
  ...
  _project(frameState) {
    ...
    const flowPaths = flows.map((f, i) => {
      ...
      const pts = curvePoints(px.get(f.origin), px.get(f.dest), { bend: 0.2, samples: 24 });
 ...
 * TableLoader 의 파서만 빌려 쓰고 그 상태(this.data)는 건드리지 않는다 (테이블 결합 패널과 충돌 방지).
 */
...
export async function readFlowFile(file) {
  ...
  const parsed = isExcel ? tableLoader.parseXLSX(raw) : tableLoader.parseCSV(raw);
...
class FlowPanel {
    ...
    return `
      <div class="flow-section">
        ...
        <div class="flow-checks">
          ...
          <label><input type="checkbox" id="flow-dark" ${mapManager.getBasemap() === 'CARTO_DARK' ? 'checked' : ''}> 어두운 배경지도</label>
        </div>
      </div>`;
  }
  ...
  _bind() {
    ...
    $('flow-dark').addEventListener('change', (e) => mapManager.setBasemap(e.target.checked ? 'CARTO_DARK' : 'CARTO_LIGHT'));
  }
...
```bash
git add src/ui/panels/FlowPanel.js src/ui/layout/AppLayout.js src/main.js src/styles/main.css
...
- [ ] **Step 2: BuiltinDataManager.loadPracticeDataset** — `if (dataset.type === 'attribute' || dataset.type === 'coordinate')` 를

```js
    if (dataset.type === 'attribute' || dataset.type === 'coordinate' || dataset.type === 'flow') {
```
로 바꾼다 (같은 XLSX/CSV 파싱, `type: 'flow'` 로 돌아온다). 주석 `type: 'spatial' | 'attribute' | 'coordinate' | 'raster'` 에 `'flow'` 를 더한다.
