...
```js
    if (dataset.type === 'flow') {
      ...
      const { headers, data } = /\.csv$/i.test(dataset.file)
        ? parseFlowCsv(new TextDecoder('utf-8').decode(buf))
        : parseFlowXlsx(buf);
      return { type: 'flow', headers, data, fileName: dataset.name, dataset };
    }
```
파일 상단 import 에 `import { parseFlowXlsx, parseFlowCsv } from '../loaders/FlowLoader.js';` 를 더하고, 주석 `type: 'spatial' | 'attribute' | 'coordinate' | 'raster'` 에 `'flow'` 를 더한다.
