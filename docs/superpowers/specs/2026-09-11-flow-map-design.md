...
- **흐름**: 기본은 **직선**(flowmap.blue). 진행 방향 오른쪽으로 자기 머리 폭의 절반만큼 비켜 놓아 A→B 와 B→A 가
  한 띠처럼 나란히 붙고 화살 머리가 마주 본다 — 호로 그리면 왕복마다 선이 둘로 갈라져 어지럽다(사용자 피드백).
  스타일 "곡선으로 그리기"를 켜면 2차 베지어 호. 어느 쪽이든 **꼬리(출발) 가늘게 → 머리(도착) 굵게** 폭을 보간해 그린다.
  굵기 = `sqrt(count)` 비례, 최대 굵기는 스타일 값(기본 12px). 색은 양에 따른 순차 램프 + 투명도.
- **그리는 순서**: 작은 흐름 먼저, 큰 흐름 나중(위). "모두 표시"여도 큰 흐름이 묻히지 않는다.
- **위치 원**: 반지름 = `sqrt(유입+유출)` 비례(최소 3px, 최대 스타일 값). 기본(`circleMode: 'net'`)은 채움색이 순유입(+)/순유출(−) 두 색,
  `'inout'`이면 한 원 안에 유입(초록)·유출(빨강) 비율 파이 — 둘 다 보이면서 원은 하나(사용자 요청).
  라벨은 위치가 많으면 총량 상위만(겹침 방지), 줌 인 할수록 더 보인다.
- **애니메이션**: 기본은 선 위에 진행 방향 화살표(＞)를 26px 간격으로 늘어놓고 위상을 매 프레임 밀어 출발→도착으로 흐르게 한다
  (`arrowHeads`, 끄면 점선 `lineDashOffset`). 애니메이션이 꺼져 있어도 화살표는 정지 상태로 남아 방향을 보여 준다.
