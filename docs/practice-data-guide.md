...
실습용 파일을 `public/data/builtin/practice/<폴더명>/` 에 넣습니다.
현재 폴더(실습 유형)는 `Point Data(점)` · `Line Data(선)` · `Area Data(면)` · `Attribute Data(속성정보)` 네 가지이며,
파일은 각각 `practice/Point Data/`, `practice/Attribute Data/` 처럼 유형별 하위 폴더에 둡니다.
...
- `file` — `public/data/builtin/` 기준 상대 경로 (예: `practice/Area Data/schools.geojson`)
