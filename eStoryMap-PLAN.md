...
│  │  ├─ CloudSync.js        // projects 테이블 read(e-GIS 연동) + storymaps 저장
...
**클라우드 동기화 (옵션, 로그인 시):**
- 저장 기본값은 로컬 `.esm`. 로그인 상태면 "클라우드에 저장" 토글로 Supabase `storymaps` 테이블에도 upsert(§6b).
...
**권한:** `projects`는 읽기만(내 것). eStoryMap 결과물은 별도 `storymaps` 테이블(§6b)에 저장 → e-GIS 데이터 오염 없음.
