...
### 📂 데이터 불러오기
- **실습 데이터**: 데이터 형태별 다섯 섹션 — Point(점) / Line(선) / Area(면, 행정경계 폴더: 시도·시군구·서울 자치구·세계 국가 GeoJSON) / Raster(래스터, 광역자치단체별 DEM GeoTIFF) / Attribute(속성정보, 테이블 결합용 XLSX·CSV). 점·선·면·속성은 `public/data/builtin/practice_catalog.json`에 직접 등록([가이드](docs/practice-data-guide.md)), 래스터는 `npm run catalog`로 자동 생성
...
```bash
# 래스터: public/data/builtin/raster/ 에 GeoTIFF 파일을 넣고
npm run catalog
# 점·선·면·속성: public/data/builtin/practice/<Point|Line|Area|Attribute Data>/ 에 넣고
