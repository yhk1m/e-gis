...
6. **Electron 패턴**: `vite-plugin-electron/simple`로 `electron/main.js`·`preload.js`를 번들(Vanilla JS). `package.json`에 `"type": "module"`을 두지 않아 번들된 main이 CJS로 실행되게 한다(vite-plugin-electron 표준 구성). 보안: `contextIsolation: true` + `nodeIntegration: false` + **sandbox 기본 활성**. 렌더러는 로컬 자체 콘텐츠만 로드하고 preload는 `contextBridge`/`ipcRenderer`만 쓰므로 sandbox를 끄지 않는다.
...
- `npm run dev` 시 vite-plugin-electron이 `electron/main.js`를 CJS로 번들해 실행하는지 → Task 2 스모크에서 확인. `package.json`에 `"type":"module"`을 넣지 않은 것이 핵심(vite-plugin-electron이 main을 CJS로 번들). sandbox 기본 활성 상태에서 `window.egisFS`가 정상 노출되는지도 첫 GUI 스모크에서 확인.
