...
export class CameraAnimator {
   ...
   * @param {{zoomForView?:(normZoom:number)=>number}} [opts] - 저장 정규화 줌 → 실제 줌 변환
   *   (미지정 시 항등 — 캔버스 폭 무관 슬라이드 extent 유지용, MapView.toRawZoom 주입)
   */
  constructor(view, { zoomForView } = {}) {
    this.view = view;
    this.zoomForView = typeof zoomForView === 'function' ? zoomForView : (z) => z;
  }

  /**
   * 카메라로 비행. 연속 전환(연타) 시 이전 비행은 취소된다.
   * @param {{center:number[], zoom:number}|null} camera - center는 EPSG:4326 [경도, 위도], zoom은 정규화 줌
  ...
  flyTo(camera, { duration = 800 } = {}) {
    ...
    this.view.animate({
      ...
      zoom: this.zoomForView(camera.zoom),
      duration,
      easing: inAndOut,
    });
  }
