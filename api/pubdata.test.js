...
describe('목록 (?list=1)', () => {
  it('학생에게 보여줄 정보만 준다', async () => {
    const res = await handle({ list: '1' }, { fetchFn: fakeFetch({}), key: KEY });
  ...
  it('키가 없어도 목록은 볼 수 있다', async () => {
    const res = await handle({ list: '1' }, { fetchFn: fakeFetch({}), key: '' });
...
describe('허용 목록', () => {
  it('id가 없으면 400', async () => {
    const res = await handle({}, { fetchFn: fakeFetch({}), key: KEY });
  ...
  it('카탈로그에 없는 id는 400이고 원본을 부르지 않는다', async () => {
    ...
    const res = await handle({ id: '남의API' }, { fetchFn, key: KEY });
  ...
  it('필수 파라미터가 빠지면 400', async () => {
    ...
    const res = await handle(query, { fetchFn: fakeFetch({}), key: KEY });
  ...
  it('카탈로그에 없는 파라미터는 원본 요청에 섞이지 않는다', async () => {
    ...
    await handle({ ...validQuery(), 이상한파라미터: 'x' }, { fetchFn, key: KEY });
...
describe('데이터 조회', () => {
  it('정규화된 결과와 캐시 헤더를 준다', async () => {
    ...
    const res = await handle(validQuery(), { fetchFn, key: KEY });
  ...
  it('요청 URL에 서비스키를 붙인다', async () => {
    ...
    await handle(validQuery(), { fetchFn, key: KEY });
  ...
  it('서버에 키가 없으면 원본을 부르지 않고 안내한다', async () => {
    ...
    const res = await handle(validQuery(), { fetchFn, key: '' });
